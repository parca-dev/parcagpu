...
void correlation_filter_insert(CorrelationFilterHandle filter,
...
bool correlation_filter_check_and_remove(CorrelationFilterHandle filter,
                                         uint32_t correlation_id);
...
// Thread-safe: can be called from multiple threads concurrently
void graph_correlation_map_insert(GraphCorrelationMapHandle map,
                                  uint32_t correlation_id);

// Start a new processing cycle - clears the appropriate slot for all entries
// Thread-safe
void graph_correlation_map_cycle_start(GraphCorrelationMapHandle map,
                                       uint32_t cycle);
...
// Thread-safe
bool graph_correlation_map_check_and_mark_seen(GraphCorrelationMapHandle map,
                                               uint32_t correlation_id,
                                               uint32_t cycle);

// End processing cycle - removes entries that haven't seen kernels in 2
// consecutive cycles Thread-safe
...
void graph_correlation_map_get_stats(GraphCorrelationMapHandle map,
                                     size_t *size, size_t *oldest_age);
